...
const dates = [];
const totalDays = 30; // 1 bulan penuh (bisa ubah sesuai kebutuhan)

// Loop untuk menghasilkan tanggal
for (let i = 0; i < totalDays; i++) {
    const date = new Date(today);  // Copy objek 'today'
    date.setDate(today.getDate() + i);  // Menambahkan hari ke tanggal sekarang
    ...
    datesElement.innerHTML += `
        <div class="swiper-slide !w-fit py-[2px]">
            <label class="relative flex flex-col items-center justify-center w-fit rounded-3xl p-[14px_20px] gap-3 bg-white border border-white hover:border-[#91BF77] has-[:checked]:ring-2 has-[:checked]:ring-[#91BF77] transition-all duration-300">
                <img src="/assets/images/icons/calender.svg" class="w-8 h-8" alt="icon">
